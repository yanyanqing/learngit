...
// the updated comment.
func (j *JIRAClient) UpdateComment(issue jira.Issue, id string, comment github.IssueComment, gClient github.Client) error {
	user, err := GetUser(gClient, comment.User.GetLogin())
	if err != nil {
...
func (j *JIRAClient) CreateComment(issue jira.Issue, comment github.IssueComment, gClient github.Client) (jira.Comment, error) {
	user, err := GetUser(gClient, comment.User.GetLogin())
	if err != nil {
...
func GetUser(client github.Client, login string) (github.User, error) {
	u, _, err := client.Users.Get(context.Background(), login)
