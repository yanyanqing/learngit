def call(BUILD_BRANCH) {
    ...
    catchError {
        stage('Build') {
            ...
            builds["linux-amd64-centos7"] = {
                node('centos7_build') {
                    def ws = pwd()
                    dir("${ws}/go/src/github.com/pingcap/pd") {
                        // checkout scm
                        ...
                        filemgr-linux64 --action mput --bucket pingcap-dev --nobar --key builds/pingcap/pd/${githash_centos7}/centos7/pd-server.tar.gz --file pd-server.tar.gz
                        """

                        // update refs
                        writeFile file: 'sha1', text: "${githash_centos7}"
                        sh "filemgr-linux64 --action mput --bucket pingcap-dev --nobar --key refs/pingcap/pd/${BUILD_BRANCH}/centos7/sha1 --file sha1"

                        // cleanup
                        sh "rm -rf sha1 pd-server.tar.gz config.cfg"
                    }
                }
            }
            ...
            builds["linux-amd64-centos6"] = {
                node('centos6_build') {
                    ...
                    dir("${ws}/go/src/github.com/pingcap/pd") {
                        ...
                        cp ~/bin/config.cfg ./
                        tar czvf pd-server.tar.gz bin/*
                        filemgr-linux64 --action mput --bucket pingcap-dev --nobar --key builds/pingcap/pd/${githash_centos6}/centos6/pd-server.tar.gz --file pd-server.tar.gz
                        """

                        // update refs
                        writeFile file: 'sha1', text: "${githash_centos6}"
                        sh "filemgr-linux64 --action mput --bucket pingcap-dev --nobar --key refs/pingcap/pd/${BUILD_BRANCH}/centos6/sha1 --file sha1"
    ...
    stage('Summary') {
        ...
        "Binary Download URL:" + "\n" +
        "${UCLOUD_OSS_URL}/builds/pingcap/pd/${githash_centos7}/centos7/pd-server.tar.gz" + "\n" +
        "Binary (for Centos6) Download URL:" + "\n" +
        "${UCLOUD_OSS_URL}/builds/pingcap/pd/${githash_centos6}/centos6/pd-server.tar.gz"
