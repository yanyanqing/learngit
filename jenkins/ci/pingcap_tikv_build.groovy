def call(BUILD_BRANCH) {
            ...
            builds["linux-amd64-centos7"] = {
                node('centos7_build') {
                    ...
                    dir("${ws}/go/src/github.com/pingcap/tikv") {
                        ...
                        GOPATH=${ws}/go:$GOPATH ROCKSDB_SYS_STATIC=1 make release
                        """
                        ...
                        filemgr-linux64 --action mput --bucket pingcap-dev --nobar --key builds/pingcap/tikv/${githash_centos7}/centos7/tikv-server.tar.gz --file tikv-server.tar.gz
                        """

                        // update refs
                        writeFile file: 'sha1', text: "${githash_centos7}"
                        sh "filemgr-linux64 --action mput --bucket pingcap-dev --nobar --key refs/pingcap/tikv/${BUILD_BRANCH}/centos7/sha1 --file sha1"
            ...
            builds["linux-amd64-centos6"] = {
                node('centos6_build') {
                    ...
                    dir("${ws}/go/src/github.com/pingcap/tikv") {
                        ...
                        sh """
                        rustup override set $RUST_TOOLCHAIN_BUILD
                        scl enable devtoolset-4 python27 "GOPATH=${ws}/go:$GOPATH ROCKSDB_SYS_STATIC=1 make release"
                        """
                        ...
                        cp ~/bin/config.cfg ./
                        tar czvf tikv-server.tar.gz bin/*
                        filemgr-linux64 --action mput --bucket pingcap-dev --nobar --key builds/pingcap/tikv/${githash_centos6}/centos6/tikv-server.tar.gz --file tikv-server.tar.gz
                        """

                        // update refs
                        writeFile file: 'sha1', text: "${githash_centos6}"
                        sh "filemgr-linux64 --action mput --bucket pingcap-dev --nobar --key refs/pingcap/tikv/${BUILD_BRANCH}/centos6/sha1 --file sha1"
    ...
    stage('Summary') {
        ...
        "Binary Download URL:" + "\n" +
        "${UCLOUD_OSS_URL}/builds/pingcap/tikv/${githash_centos7}/centos7/tikv-server.tar.gz" + "\n" +
        "Binary (for Centos6) Download URL:" + "\n" +
        "${UCLOUD_OSS_URL}/builds/pingcap/tikv/${githash_centos6}/centos6/tikv-server.tar.gz"
