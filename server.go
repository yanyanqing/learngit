...
func (s *Server) init() error {
	...
	client := &github.BasicAuthTransport{
		Username: s.cfg.GConfig.Username,
		Password: s.cfg.GConfig.Password,
	}
	s.gClient = github.NewClient(client.Client())
...
func (s *Server) demuxEvent(eventType, eventGUID string, payload []byte, h http.Header) error {
	...
	//	}
	s.demuxExternal(eventType, l, s.gHook.Plugins.Config().ExternalPlugins[s.cfg.JConfig.Project], payload)
...
func (s *Server) demuxExternal(eventType string, l *logrus.Entry, externalPlugins []plugins.ExternalPlugin, payload []byte) {
	for _, p := range externalPlugins {
		go func(p plugins.ExternalPlugin) {
			if err := s.syncIssues(eventType, p.Endpoint, l, payload); err != nil {
...
func (s *Server) syncIssues(eventType, endpoint string, l *logrus.Entry, payload []byte) error {
	switch eventType {
	case "issues":
		...
		go s.compareIssues(endpoint, l, i)
	case "issue_comment":
		var ic github.IssueCommentEvent
		if err := json.Unmarshal(payload, &ic); err != nil {
			return errors.Trace(err)
		}
		go s.compareIssueComment(l, ic)
	default:
...
func (s *Server) compareIssues(endpoint string, l *logrus.Entry, gIssue github.IssueEvent) error {
	...
	log.Infof("len(issue) %v found %v", len(jIssues), found)
	if !found {
		if err = s.createIssue(*gIssue.Issue, s.gClient, s.jClient); err != nil {
			log.Errorf("Error creating issue Error: %v", err)
		}
	} else if len(jIssues) > 0 {
		if err = s.updateIssue(*gIssue.Issue, jIssues[0], s.gClient, s.jClient); err != nil {
...
func (s *Server) compareIssueComment(l *logrus.Entry, gComment github.IssueCommentEvent) error {
	...
	log.Infof("issue self %v, Fileld.Comments %v", jIssue.ID, jIssue.Fields.Comments)
